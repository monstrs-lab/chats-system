import type { TLExtendedSchemaParam } from '@monstrs/mtproto-tl-types'

import { TLConstructor }              from '@monstrs/mtproto-tl-core'

interface UrlAuthResultRequestValues {
  flags: any
  requestWriteAccess: boolean
  bot: any
  domain: string
}

export class UrlAuthResultRequest extends TLConstructor<UrlAuthResultRequestValues> {
  static override CONSTRUCTOR_ID: number = -1831650802
  static override PARAMS: Array<TLExtendedSchemaParam> = [
    {
      name: 'flags',
      type: '#',
      isVector: false,
      isFlag: false,
      skipConstructorId: false,
      flagGroup: 0,
      flagIndex: -1,
      flagIndicator: true,
      useVectorId: false,
    },
    {
      name: 'request_write_access',
      type: 'true',
      isVector: false,
      isFlag: true,
      skipConstructorId: true,
      flagGroup: 1,
      flagIndex: 0,
      flagIndicator: false,
      useVectorId: false,
    },
    {
      name: 'bot',
      type: 'User',
      isVector: false,
      isFlag: false,
      skipConstructorId: false,
      flagGroup: 0,
      flagIndex: -1,
      flagIndicator: false,
      useVectorId: false,
    },
    {
      name: 'domain',
      type: 'string',
      isVector: false,
      isFlag: false,
      skipConstructorId: true,
      flagGroup: 0,
      flagIndex: -1,
      flagIndicator: false,
      useVectorId: false,
    },
  ]

  get flags(): any {
    return this.getParamValue<any>('flags')
  }

  get requestWriteAccess(): boolean {
    return this.getParamValue<boolean>('requestWriteAccess')
  }

  get bot(): any {
    return this.getParamValue<any>('bot')
  }

  get domain(): string {
    return this.getParamValue<string>('domain')
  }
}
