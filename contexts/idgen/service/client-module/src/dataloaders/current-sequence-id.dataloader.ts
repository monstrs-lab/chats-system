...
@Injectable()
export class CurrentSequenceIdDataLoader {
  ...
  constructor(
    ...
    this.dataloader = new DataLoader<SequenceQuery, Sequence>(async (queries) =>
      this.getCurrentSequenceIds(queries))
  }
  ...
  protected async getCurrentSequenceIds(
    queries: ReadonlyArray<SequenceQuery>
  ): Promise<Array<Sequence>> {
    const { sequences } = await this.client.getCurrentSequenceIds({
